...
function parseArgs(): { proxyPort: number; targetPort: number } {
...
function getRegisteredRoutes(): ChannelRoute[] {
  ...
  try {
    ...
    if (fs.existsSync(compiledPath)) {
      ...
      if (Array.isArray(manifest.channels)) {
        return manifest.channels.map((c: any) => ({
          name: c.name,
          kind: c.kind ?? "unknown",
  ...
  try {
    const info: EveInfoOutput = JSON.parse(jsonOutput.trim());
...
function inspectConnectors(routes: ChannelRoute[]): void {
...
function startProxy(routes: ChannelRoute[], proxyPort: number, targetPort: number): void {
  ...
  const server = http.createServer((clientReq, clientRes) => {
    ...
    if (match) {
      ...
      console.log(
        `Request to ${pathname} [${reqMethod}] did not match any eve channel.\nRegistered paths are:\n${routes
